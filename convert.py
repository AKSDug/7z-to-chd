...
def parse_args():
    ...
    parser.add_argument(
        "--keep", "-k", choices=["yes", "no"], help="Keep original files after conversion"
    )
    parser.add_argument(
        "--threads",
        "-t",
        type=int,
        help="Maximum number of concurrent operations (0 for CPU count)",
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose logging")
    parser.add_argument(
        "--state-file", help="Path to playlist state file (for resuming interrupted conversions)"
    )
    parser.add_argument(
        "--skip-playlist-scan", action="store_true", help="Skip initial scan for existing playlists"
    )
...
def prompt_user_input(args):
    ...
    print(f"{colorama.Fore.CYAN}7z-to-CHD Converter v{__version__}{colorama.Style.RESET_ALL}")
...
def batch_process(
    ...
    if not skip_playlist_scan:
        print(
            f"{colorama.Fore.YELLOW}Scanning for existing multi-disc games...{colorama.Style.RESET_ALL}"
        )
    ...
    logger.info(
        f"Found approximately {len(unique_games)} unique games in {len(archive_files)} archives"
    )

    # Process archives
    print(
        f"{colorama.Fore.YELLOW}Processing {len(archive_files)} archives...{colorama.Style.RESET_ALL}"
    )
    ...
    print(
        f"{colorama.Fore.YELLOW}Creating playlists for multi-disc games...{colorama.Style.RESET_ALL}"
    )
    ...
    incomplete_series = sum(
        1 for info in series_status.values() if not info["is_complete"] and info["disc_count"] > 1
    )
...
def display_summary(statistics):
    ...
    if statistics["archives_found"] > 0:
        archives_success_rate = (
            statistics["archives_processed"] / statistics["archives_found"]
        ) * 100
...
def main():
    ...
    try:
        # Get user input
        source_dir, dest_dir, keep_files, max_workers, state_file, skip_playlist_scan = (
            prompt_user_input(args)
        )

        # Process files
        statistics = batch_process(
            source_dir, dest_dir, keep_files, max_workers, state_file, skip_playlist_scan
        )
