...
class Extractor:
    ...
    def __init__(self, temp_dir=None, max_workers=None, output_dir=None):
        ...
        logger.debug(
            f"Extractor initialized with temp_dir={self.temp_dir}, max_workers={max_workers}"
        )
    ...
    def analyze_archive(self, archive_path):
        ...
        if self.output_dir and self.output_dir.exists():
            ...
            if base_game and disc_number:
                ...
                if chd_path.exists():
                    logger.info(
                        f"Archive {archive_path.name} can be skipped - CHD already exists: {chd_path.name}"
                    )
        ...
        try:
            ...
            if self.output_dir and analysis["has_disc_images"] and convertible_files:
                ...
                if can_skip:
                    logger.info(
                        f"Archive {archive_path.name} can be skipped - all CHDs already exist"
                    )
            ...
            logger.info(
                f"Archive analysis complete: {archive_path.name} - "
    ...
    def check_system_resources(self):
        ...
        logger.debug(
            f"System resources: {resources['memory_percent']}% memory used, "
    ...
    def calculate_optimal_workers(self, archive_analyses):
        ...
        memory_based_medium = max(
            1, int(available_memory_gb / 2)
        )  # 2GB per medium-complexity worker
        ...
        logger.info(
            f"Optimal workers: minimal={workers['minimal']}, low={workers['low']}, "
    ...
    def extract_multiple(self, archive_paths, target_dir=None):
        ...
        for analysis in archive_analyses:
            ...
            if analysis["can_skip"]:
                ...
                if (
                    analysis["base_game"]
                    and analysis["disc_number"]
                    and analysis["base_game"] in game_series
                ):
                    ...
                    if (
                        len(series_discs) >= 2
                        and analysis["base_game"] not in self.completed_game_series
                    ):
                        # Mark for immediate M3U creation
                        self._check_and_notify_series_completion(
                            analysis["base_game"], series_discs
                        )
        ...
        logger.info(
            f"Archive complexity breakdown: "
        ...
        if minimal_complexity:
            ...
            minimal_results = self._extract_batch(
                minimal_complexity, target_base, optimal_workers["minimal"]
            )
        ...
        if low_complexity:
            ...
            low_results = self._extract_batch(low_complexity, target_base, optimal_workers["low"])
        ...
        if medium_complexity:
            ...
            medium_results = self._extract_batch(
                medium_complexity, target_base, optimal_workers["medium"]
            )
        ...
        if high_complexity:
            logger.info(
                f"Processing {len(high_complexity)} high complexity archives sequentially..."
            )
            for archive in high_complexity:
                ...
                try:
                    ...
                    if resources["memory_percent"] > 75:
                        logger.info(
                            f"Memory usage at {resources['memory_percent']}%, waiting briefly"
                        )
        ...
        logger.info(
            f"Extraction complete: {successful}/{len(archive_paths) - len(skippable)} extracted, "
                ...
                if extension in supported_extensions:
                    ...
                    if self.output_dir:
                        ...
                        if chd_path.exists():
                            ...
                            # Register with PlaylistManager if available
                            if self.playlist_manager:
                                self.playlist_manager.register_disc(
                                    chd_path, update_playlists=False
                                )
                            # Legacy tracking
                            else:
    ...
    def _check_and_notify_series_completion(self, base_game, disc_numbers):
        ...
        if len(disc_numbers) >= 2 and base_game not in self.completed_game_series:
            logger.info(
                f"Detected complete game series: {base_game} with discs {sorted(disc_numbers)}"
            )
