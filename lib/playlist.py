...
class PlaylistManager:
    ...
    def __init__(self, output_dir: Optional[Union[str, Path]] = None, 
        ...
        self.game_series = defaultdict(list)  # Maps base game name to list of (chd_path, disc_num) tuples
        ...
        self.recently_updated = set()         # Tracks series updated in current session to prevent redundant updates
        ...
        for m3u_file in self.output_dir.glob("*.m3u"):
            if m3u_file.is_file():
                ...
                try:
                    ...
                    logger.debug(f"Analyzed existing M3U: {m3u_file.name}, found {len(chd_files)} disc entries")
    ...
    def _load_state(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Loaded playlist state from {self.state_file} (last updated: {last_updated})")
    ...
    def _update_series_signature(self, base_game: str) -> str:
        ...
        disc_info = sorted([(disc_num, path.name) for path, disc_num in self.game_series[base_game]])
    ...
    def register_disc(self, chd_path: Union[str, Path], update_playlists: bool = True) -> Optional[str]:
        """
    ...
    def _find_related_discs(self, base_game: str) -> None:
        ...
        pattern_base = base_game.replace('(', r'\(').replace(')', r'\)').replace('[', r'\[').replace(']', r'\]')
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if not already_tracked:
                        logger.debug(f"Found additional disc {disc_num} for {base_game}: {chd_file.name}")
    ...
    def register_multiple_discs(self, chd_paths: List[Union[str, Path]], 
                             update_playlists: bool = True) -> Dict[str, int]:
        """
        ...
        for base_game, discs in game_groups.items():
            ...
            if update_playlists and len(self.game_series[base_game]) > 1:
                ...
                if is_complete or highest_processed == max_disc:
                    logger.info(f"Creating/updating playlist for {base_game} - {len(disc_numbers)} discs found")
                    self.update_playlist(base_game)
    ...
    def _update_user_customized_playlist(self, base_game: str, m3u_path: Path) -> Path:
        ...
        try:
            ...
            logger.info(f"Updated user-customized playlist {m3u_path} with {len(new_entries)} new discs")
            return m3u_path
    ...
    def _create_standard_playlist(self, base_game: str, m3u_path: Path) -> Path:
        ...
        try:
            ...
            if missing_discs:
                ...
                logger.warning(f"Creating playlist for {base_game} with missing disc(s): {missing_str}")
    ...
    def scan_directory(self, directory: Optional[Union[str, Path]] = None, update_all: bool = False) -> Dict[str, Path]:
        """
    ...
    def check_for_incomplete_series(self, min_discs: int = 2, 
                                 expected_max_discs: int = 6, 
                                 force_scan: bool = False) -> Dict[str, Path]:
        """
                        ...
                        if has_consecutive:
                            ...
                            if m3u_path:
                                ...
                                if clean_name not in self.created_playlists:
                                    logger.info(f"Created playlist for game series: {base_game} (discs: {disc_nums})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Create and manage M3U playlists for multi-disc games')
